...
if (isset($_POST['ftags'])) {
    $post_tags = $_POST['ftags'];
//CHECK IF NO NEW TAGS AND NO CHECKED TAGS
} elseif (empty($_POST['newtag']) && empty($_POST['ftags'])) {
    $post_tags = array("19"); //None defined
}

//ADD BOOKMARK, NEW TAG, AND MAP TOGETHER
$sql_insert_f_bookmarks = 
"INSERT INTO f_bookmarks (bmTitle, bmURL, bmShortDesc, bmNotes)
VALUES ('$postname', '$posturl', '$shortdesc', '$postnotes')";

if ($connection->query($sql_insert_f_bookmarks) === TRUE) {
    ...
    if (!empty($_POST['newtag'])) {
        ...
        if ($connection->query($sql_insert_newtag) === TRUE) {
            ...
            "INSERT INTO f_map (bmMapID, tagMapID)
